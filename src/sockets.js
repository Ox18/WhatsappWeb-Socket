...
export default (io) => {
  ...
  var messages = (await MessageServiceImpl.getAllMessages()).map((message) =>
    message.get()
  );
  var chats = (await ChatServiceImpl.getChats()).map((chat) => chat.get());
  ...
  client.on("message", async (data) => {
    ...
    if (statusIsWaiting) {
      if (body === "/finalizar") {
        ...
        data.reply("¡Hasta pronto! Se finalizo la conversación con el asesor.");
      }
    } else {
